...
import io.github.gabrielshanahan.structmess.coroutine.StandardEventLoopStrategy
...
class PendingCoroutineRunSqlTest {
    ...
    fun <T> List<T>.isEqualToInAnyOrder(other: List<T>, message: String) =
        assertTrue(containsAll(other) && other.containsAll(this), message)

    @Nested
    inner class CandidateSeenTest {

        fun List<Row>.assertNoSeen() =
            assertTrue(isEmpty(), "no SEEN should be picked up, but got $this")
    ...
    inner class LatestSuspendedTest {
        ...
        fun List<Row>.assertSuspendedAbsent() =
            assertTrue(isEmpty(), "SUSPENDED row shouldn't be present")

        fun List<Row>.assertSuspendedStepIs(step: String) =
            single().getString("step").also {
                assertTrue(it == step, "SUSPENDED step should be $step, but is $it")
            }
    ...
    inner class ChildEmissionsInLatestStep {
        ...
        fun List<Row>.assertNothingWasEmitted() =
            assertTrue(isEmpty(), "no EMITTED rows should be present")
    ...
    inner class ChildRollbackEmissionsInLatestStep {
        fun List<Row>.assertNothingWasEmitted() =
            assertTrue(isEmpty(), "no ROLLBACK_EMMITED rows should be present")
    ...
    inner class CandidateSeensWaitingToBeProcessed {
        ...
        inner class RegistryStrategy {
            ...
            val registryStrategy =
                StandardEventLoopStrategy(ZonedDateTime.now()) {
                    mapOf(
    ...
    inner class SeenForProcessing {
        ...
        fun List<Row>.assertSeenIsFor(messageId: UUID) =
            single().getUUID("message_id").also {
                assertTrue(it == messageId, "SEEN message_id should be $messageId, but was $it")
            }
                    ...
                    .withTransaction { connection ->
                        connection
                            ...
                            .invoke { result ->
                                ...
                                verify(seenForProcessing) { result -> result.assertNoSeen() }
                            }
                    }
                    .emitOn(Infrastructure.getDefaultWorkerPool())
                    .invoke { result ->
    ...
    inner class FinalSelect {
        ...
        fun List<Row>.asResult() =
            single().let {
                FinalSelectResult(
                    childRolledBackExceptions = it.getJsonArray("child_rolled_back_exceptions"),
                    childRollbackFailedExceptions =
                        it.getJsonArray("child_rollback_failed_exceptions"),
                    rollingBackException = it.getJsonObject("rolling_back_exception"),
                )
            }
            ...
            emitRootMessage(rootTopic).coroutineProgress(rootHandler) {
                ...
                verify(finalSelect) { result ->
                    result.asResult().apply {
                        assertFalse(anyChildRolledBack)
                        assertFalse(anyChildRollbackFailed)
                        assertFalse(rollingBack)
                    }
                }
            }
        }
        ...
        fun `picks up children rollback failures`() {
            ...
            emitRootMessage(rootTopic).coroutineProgress(rootHandler) {
                ...
                verify(finalSelect) { result ->
                    result.asResult().apply {
                        assertTrue(anyChildRolledBack)
                        assertTrue(anyChildRollbackFailed)
                        assertFalse(rollingBack)
                    }
                }
            }
    ...
    fun emitRootMessage(topic: String, key: String = "key", value: String = "value"): Message {
        val sqlTestUtils = SqlTestUtils(pool)
    ...
    inner class CoroutineProgressBuilder(
        ...
        fun seen() = SqlTestUtils(pool).seen(message.id, distributedCoroutineIdentifier, scope)
