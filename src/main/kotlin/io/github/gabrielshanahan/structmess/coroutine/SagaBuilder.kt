...
class SagaBuilder(val name: String, val childStrategy: ChildStrategy) {
    ...
    fun build(): DistributedCoroutine =
        DistributedCoroutine(DistributedCoroutineIdentifier(name), steps, childStrategy)
}
...
): DistributedCoroutine = SagaBuilder(name, childStrategy).apply(block).build()
