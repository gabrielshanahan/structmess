...
class MessageEventRepository(private val objectMapper: ObjectMapper) {
    ...
    /** Inserts an EMITTED event for a global scope. */
    ...
    /** Inserts an EMITTED event for a specific scope. */
    ...
    /** Inserts a ROLLBACK_EMITTED event. */
    fun insertRollbackEmittedEvent(
        ...
        connection
            ...
            .execute(
                Tuple.tuple(
                    listOf(messageId, cooperationLineage.toTypedArray<UUID>(), exceptionJson)
                )
            )
            .replaceWith(Unit)

    /** Inserts a CANCELLATION_REQUESTED event. */
    ...
    /** Inserts a generic message event. */
    ...
    /** Inserts ROLLBACK_EMITTED events for all emissions in a step. */
    ...
    /** Marks seen or rolling back for a coroutine. */
    ...
    /** Fetches a coroutine for processing. */
    fun fetchPendingCoroutineRun(
        ...
        connection
            .preparedQuery(finalSelect(cooperationHierarchyStrategy).build())
            ...
            .flatMap { rowSet ->
                ...
                if (row == null) {
                    ...
                    // we fetched earlier. Therefore, we mitigate this by rerunning the step which
                    // just once we have acquired the lock.
                    //
                    ...
                    // acquired the lock, which is exactly how double-checked locking is
                    // implemented.
                    //
                    // For a different example of this behavior, take a look at, e.g.,
                    // https://postgrespro.com/list/thread-id/2470837
                    connection
                        .preparedQuery(finalSelect(cooperationHierarchyStrategy, true).build())
                    ...
                    Uni.createFrom()
                        .item(
                            PendingCoroutineRun(
                                ...
                                context = row.getString("latest_context"),
