    ...
    terminatedChildRollingBacks.appendAs(
        ...
        """
                ...
                WHERE
                    -- No emissions
                    (
                        NOT EXISTS (SELECT 1 FROM child_emissions_in_latest_step WHERE child_emissions_in_latest_step.cooperation_lineage = candidate_seens.cooperation_lineage) 
                        AND 
                        NOT EXISTS (SELECT 1 FROM child_rollback_emissions_in_latest_step WHERE child_rollback_emissions_in_latest_step.cooperation_lineage = candidate_seens.cooperation_lineage)
                    )
                    OR
                    -- or no rollback emissions present, normal emissions present, and all completed
                    (
                        ...
                        AND
                        NOT EXISTS ( -- no SEEN's missing, according to strategy
                            SELECT 1
                                FROM child_emissions_in_latest_step
                    ...
                    OR
                    -- or rollback emissions present, and all completed
                    (
                        ...
                        AND
                        NOT EXISTS ( -- no ROLLING_BACK's missing, according to strategy
                            SELECT 1
                                FROM child_rollback_emissions_in_latest_step
    ...
    if (!secondRunAfterLock) {
        ...
        candidateSeensWaitingToBeProcessed(cooperationHierarchyStrategy)
            .appendAs(
                ...
                WHERE candidate_seens_waiting_to_be_processed.message_id = $2
            """
                    .trimIndent(),
            )
    }
