...
CREATE TABLE IF NOT EXISTS message_event (
    ...
    type VARCHAR(50) NOT NULL CHECK (      --TODO: Change to enum type
        type IN (
                 ...
                 'SUSPENDED',
                 'COMMITTED',
                 'ROLLING_BACK', --TODO: Doc that rollback can only be initiated from something that's running, or from the top (in the case of cancellations).
    ...
    -- 10:52:40.337 [70] LOG:  statement: ROLLBACK <--- lock released
    ...
    -- event loop triggering the processing of a message in .334 via connection [70]. We can then see us inserting the ROLLING_BACK at .336, in its own
    -- single-statement-transaction, via connection [86]. Since the statement is its own transaction, created_at of ROLLING_BACK will be .366, and, at .337,
    -- the outer transaction rolls back and releases the FOR UPDATE lock. Only at .338 does the periodically triggered processing get around to running the
    -- SELECT FOR UPDATE, at which point the record is already unlocked, everything proceeds normally, and we insert and commit a ROLLED_BACK at .339.
    -- However, if we used CURRENT_TIMESTAMP, the created_at of ROLLED_BACK would be .334 - the time the transaction started. As a consequence, when ordered
    -- by created_at, the ROLLED_BACK would appear to have happened before the ROLLING_BACK.
    --
    ...
    -- we really do want to use the time of insertion and not the time the transaction started. This is especially true since we're never actually persisting more
    -- than a single record at a time.
    --
...
CREATE INDEX IF NOT EXISTS idx_message_event_message_id ON message_event (message_id);
...
CREATE INDEX IF NOT EXISTS idx_message_event_msg_type ON message_event (message_id, type);
CREATE INDEX IF NOT EXISTS idx_message_event_handler_msg_type ON message_event (coroutine_name, message_id, type);
...
-- Comment on table
COMMENT ON TABLE message_event IS 'Stores records of message handling events in a distributed system';

-- Comments on columns
COMMENT ON COLUMN message_event.id IS 'Unique identifier for the message_event entry (UUIDv7)';
COMMENT ON COLUMN message_event.message_id IS 'Reference to the message this message_event entry relates to';
COMMENT ON COLUMN message_event.coroutine_name IS 'Name of the component/service handling the message';
COMMENT ON COLUMN message_event.coroutine_identifier IS 'Unique instance ID of the handler (e.g., pod/container ID)';
COMMENT ON COLUMN message_event.type IS 'Type of message_event event (EMITTED, PICKED_UP, HANDLED_SUCCESSFULLY, HANDLED_EXCEPTIONALLY, SUSPENDED, RESUMED)';
COMMENT ON COLUMN message_event.created_at IS 'Timestamp when the message_event entry was created';
COMMENT ON COLUMN message_event.exception IS 'Serialized exception information in a cross-system/cross-language compatible format';
